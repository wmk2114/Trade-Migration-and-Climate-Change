...
```{r}
polity_xls<- GET("http://www.systemicpeace.org/inscr/p5v2018d.xls")
...
for(j in colnames(polity)){
  polity[, j][[1]]<- sapply(polity[, j][[1]], get_rid_of)
}
```
...
```{r}
polity_2<-polity %>%
  mutate("diff_year"=eyear-byear)
```

```{r}
polity_update<- data.frame()
for(j in seq_len(nrow(polity_2))){
  row<- polity_2[j,]
  if(row$diff_year<=0){
    next
  }
  df<- data.frame()
  for(i in seq_len(row$diff_year)){
    df<- rbind(df, row)
    }
  df$year<-row$byear+1:row$diff_year
  polity_update<-rbind(polity_update, df)
}
```

```{r}
colnames(polity_update)[length(colnames(polity_update))]<- "date"
```

```{r}
polity_mk<-merge(data, polity_update, by=c("ctylabel", "date"))
...
mm1_1<- prais::prais_winsten(newtar~.-l1polity_no_xconst, data=mdf_1, index=c("date", "country"))
